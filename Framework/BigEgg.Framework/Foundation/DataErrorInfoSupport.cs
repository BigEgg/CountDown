    ...
    public sealed class DataErrorInfoSupport : IDataErrorInfo
    {
        ...
        public string this[string memberName]
        {
            get
            {
                ...
                {
                    PropertyDescriptor property = TypeDescriptor.GetProperties(this.instance)[memberName];
                    if (property == null)
                    {
                        throw new ArgumentException(string.Format(CultureInfo.CurrentCulture,
                            "The specified member {0} was not found on the instance {1}", memberName, this.instance.GetType()));
                    }
                    Validator.TryValidateProperty(property.GetValue(this.instance),
                        new ValidationContext(this.instance, null, null) { MemberName = memberName }, validationResults);
                }
